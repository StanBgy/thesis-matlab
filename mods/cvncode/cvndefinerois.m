...
% calc
fsdir0 = sprintf('%s\\%s',cvnpath('freesurfer'),subjid);  % subject FS 
...
if ~exist('overlays','var') || isempty(overlays)
    overlays = cell(1,length(crngs));
end
  ...
  for pp=1:length(mgznames)
    if ischar(mgznames{pp})
      for hh=1:length(hemis)
        ...
        alldata{pp,hh}(alldata{pp,hh}<threshs{pp}) = 0;
      end
    else
...
% trim with goodness of fit
if ~exist('views_to_trim','var') || isempty(views_to_trim)
    views_to_trim = [];
end
for pp= 1:length(views_to_trim)
    for hh=1:length(hemis)
        alldata{views_to_trim(pp),hh}(alldata{r2_view,hh}<r2_threshold)=0;
    end
end
numlh = curv.numlh;
numrh = curv.numrh;
...
while 1
  ...
  if ~isequal(viewmode,oldviewmode)
    ...
    for zz=1:size(alldata,1)
      [~,Lookup{zz},~,himgs{zz}] = cvnlookup(subjid,viewmode,cat(1,alldata{zz,:}),crngs{zz},cmaps{zz},threshs{zz},overlays{zz},[],2);
...
%     if exist('ang_view','var') & exist('ecc_view','var')
%         colormap(curfig,cmaps{ang_view});
%         colorbar('v','TickLabels',360:-36:0);
%         axh = axes('Visible','off','Position',[0.3 0 0.6 0.1]);
%         colormap(axh,cmaps{ecc_view});
%         colorbar(axh,'h','TickLabels',0:1.2:12);
%     end
%    pause(1);  % some delay is necessary in order to make sure the main ROI figure is created
  ...
  if ismember(whmode,[1 2]) % Draw | Draw-safe (only unlabeled)
    while 1
  ...
  elseif ismember(whmode,[5 6]) % Erase-safe (only from selected) | Clear
    while 1
  ...
  case 2 % Draw-safe (only unlabeled)
  ...
  case 5 % Erase-safe (only from selected)
  ...
  case 15  % Save
    ...
    while 1
      savename = sprintf("lh.%s.testrois.mgz",subjid);
      [savefilename,savepathname] = uiputfile(savename,'Save ROI labels');
      ...
      savename = sprintf("rh.%s.testrois.mgz",subjid);
      [savefilename,savepathname] = uiputfile(savename,'Save ROI labels');
      if isequal(savefilename,0) || isequal(savepathname,0)  % if user just canceled, do nothing
        fprintf('No file saved.\n');
        break;
