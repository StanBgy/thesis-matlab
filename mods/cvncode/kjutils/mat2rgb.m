...
if(options.inclusive)
    if(~isempty(options.absthreshold))
        if(size(options.absthreshold,2)==1)
            tmpalpha=abs(imgvals)>=options.absthreshold;
        elseif(size(options.absthreshold,2)==2)
            tmpalpha=abs(imgvals)>=options.absthreshold(1);
            tmpalpha=tmpalpha<=options.absthreshold(2);
        end
    elseif(~isempty(options.threshold))
        if(size(options.threshold,2)==1)
            tmpalpha=imgvals>=options.threshold;
        elseif(size(options.threshold,2)==2)
            tmpalpha=imgvals>=options.threshold(1);
            tmpalpha=tmpalpha<=options.threshold(2);
        end
    ...
    elseif(~isempty(options.threshold))
        if(size(options.threshold,2)==1)
            tmpalpha=imgvals>options.threshold;
        elseif(size(options.threshold,2)==2)
            tmpalpha=imgvals>options.threshold(1) & imgvals<options.threshold(2);
        end
    elseif(~isempty(options.overlayrange))
        tmpalpha=imgvals>options.overlayrange(1) & imgvals<options.overlayrange(2);
    end
