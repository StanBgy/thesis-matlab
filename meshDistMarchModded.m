function [varargout]=meshDistMarchModded(varargin)
...
% Change log:
% 2018/03/27 
% 2023/02: [Luis-Feilem] added optional input 'roi' to allow stop condition
% to depend on a subset of vertices and speed up computations in these
% cases.
% -----------------------------------------------------------------------
...
for q=1:1:numLoopSteps
    ...
    while 1
        ...
        D_check=DD+DE; %Distance data plus edge lenghts  % I think this stops it
        ...
        switch roiflag
            case 1
                cond1 = sum(isnan(d(roi))) == 0;
                cond2 = sum((d_previous(roi)-d(roi)).^2,'omitnan')<=toleranceLevel; %Check if converged to within tolerance;
            case 0
                cond1 = nnz(isnan(d))==nnz(isnan(d_previous));
                cond2 = sum((d_previous-d).^2,'omitnan')<=toleranceLevel; %Check if converged to within tolerance
                
        end
        
        if cond1  %Check if number of NaN's changed
            if cond2 
